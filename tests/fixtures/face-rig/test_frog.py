...
and lining sealing them), and the fangs have their own `teeth_exposed` material.
Blender coordinates: Z up, meters, the face looks down -Y, character left is +X.
"""
from agent_meshes_author import (
    JawHinge, add_jaw_open, brow_ridge_geometry, build_eye, ellipsoid_geometry, exposed_teeth_geometry, eye_hole, eye_hole_mask,
...
def build():
    ...
    for side, center in (('L', EYE_L), ('R', EYE_R)):
        ...
        # The heavy brow ridge rides the eye's mound (the skin eye_hole pushed out over the lids): browDown drops its
        # inner end over the eye.
        dome = holes[side]['mound']

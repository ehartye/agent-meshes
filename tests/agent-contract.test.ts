...
describe('agent authoring contract', () => {
  it('publishes serializable schemas and valid examples for every supported operation', () => {
    const expected = ['add', 'update', 'remove', 'bone.add', 'bone.update', 'bone.remove', 'bone.mirror', 'pose', 'pose.reset', 'bind', 'unbind', 'clip.set', 'clip.remove', 'assembly.copy', 'pose.target', 'shell.set', 'shell.remove'];

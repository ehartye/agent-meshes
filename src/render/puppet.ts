...
/** A pose offset applied on top of the rest pose and any playing clip. Rotation is XYZ Euler degrees. */
export interface PoseInput { rotation?: [number, number, number]; position?: [number, number, number] }
export interface Pose { rotation: [number, number, number]; position: [number, number, number] }

interface Rest { position: Vector3; quaternion: Quaternion }
interface Offset { position: Vector3; quaternion: Quaternion; euler: [number, number, number] }
...
export function createPuppet(gltf: { scene: Object3D; animations: AnimationClip[] }) {
  ...
  root.traverse(object => {
    if (object instanceof Bone) { bones.set(object.name, object); rest.set(object.name, { position: object.position.clone(), quaternion: object.quaternion.clone() }); }
  ...
  function apply(): void {
    for (const [name, value] of bones) { const base = rest.get(name)!; value.position.copy(base.position); value.quaternion.copy(base.quaternion); }
    for (const sampler of samplers) sampler.target[sampler.property].fromArray(sampler.interpolant.evaluate(time));
    for (const [name, offset] of offsets) { const value = bones.get(name)!; value.position.add(offset.position); value.quaternion.multiply(offset.quaternion); }
  ...
  const puppet = {
    ...
    setPose(name: string, pose: PoseInput): void {
      bone(name);
      const offset = offsets.get(name) ?? { position: new Vector3(), quaternion: new Quaternion(), euler: [0, 0, 0] as [number, number, number] };
    ...
    getPose(name: string): Pose {
      ...
      return offset ? { rotation: [...offset.euler], position: offset.position.toArray() as [number, number, number] } : { rotation: [0, 0, 0], position: [0, 0, 0] };
    },

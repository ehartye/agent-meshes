...
export async function previewHTML(name: string, glb: Uint8Array): Promise<string> {
  ...
  return `<!doctype html><html lang="en"><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Agent Meshes preview</title><style>*{box-sizing:border-box}body{margin:0;background:#dce7eb;color:#213844;font:14px 'Segoe UI',sans-serif;overflow:hidden}canvas{display:block}#stage{position:fixed;inset:0}header{position:absolute;top:25px;left:30px;pointer-events:none}header span{font-size:10px;letter-spacing:2px;color:#146b80}h1{font-size:28px;font-weight:500;margin:8px 0}footer{position:absolute;bottom:25px;left:30px;right:30px;display:flex;gap:12px;align-items:center;background:#f3f6f7e8;padding:15px;border-radius:8px;box-shadow:0 3px 20px #21384412}button,select{background:white;border:1px solid #bfd0d8;border-radius:4px;color:#213844;padding:8px;font:inherit}input{flex:1;min-width:50px;accent-color:#146b80}#status{position:absolute;top:30px;right:30px;font-size:11px;color:#50707e}#morphs{position:absolute;top:80px;right:30px;bottom:110px;width:240px;overflow:auto;background:#f3f6f7e8;padding:10px;border-radius:8px;box-shadow:0 3px 20px #21384412;font-size:11px}#morphs label{display:flex;flex-direction:column;margin:4px 0}#morphs .presets{display:flex;flex-wrap:wrap;gap:4px;margin-bottom:8px}#morphs button{padding:4px 6px;font-size:11px}[hidden]{display:none!important}@media(max-width:700px){#status{top:95px;left:30px}footer{left:10px;right:10px;gap:5px;padding:8px;font-size:11px}#morphs{top:auto;bottom:80px;left:10px;right:10px;width:auto;max-height:35vh}}</style><div id="stage"></div><header><span>AGENT MESHES / EXPORTED MODEL</span><h1 id="title"></h1></header><div id="status">Loading…</div><footer><button id="play">Pause</button><select id="clip" aria-label="Clip"></select><input id="scrub" aria-label="Time" type="range" min="0" max="1" step="0.001" value="0"><button id="download">Download GLB</button></footer><aside id="morphs" aria-label="Morphs" hidden></aside><script id="asset" type="application/json">${data}</script><script>${escape(viewer)}</script><script>${escape(shell)}</script></html>`;
}

...
**Skins and SkeletalMeshes.** Interchange builds one SkeletalMesh and one Skeleton per glTF skin. A mesh node with morph targets but no `skin` still becomes a SkeletalMesh, because morphs need one, but on a made-up one-bone skeleton named after the node (for example `Head_<hash>`). An unskinned mesh without morphs becomes a StaticMesh only in a file with no skin at all; in a file with a skin it disappears (next paragraph). So a head whose face mesh was left out of the skin imports as two SkeletalMeshes: the eyeballs with `head`/`eye_L`/`eye_R` and no morphs, and the face with every morph and one fake bone. No Unreal asset has both. `verify-unreal` therefore checks the expectations against one SkeletalMesh, the one carrying the most expected morphs, never against names pooled from several. It also reads the GLB's skins before starting Unreal (`preflight.skins`: each skin's joint names and the mesh nodes bound to it, and every mesh node's skin and morph count). A failure names the cause it finds there: no skin at all, a morph-bearing mesh node that is not skinned, or meshes bound to different skins. For a GLB with no skin, the missing `head` bone is reported as a missing skin, not as a spelling problem with the made-up `Head` bone. Bind every mesh, morph-bearing or not, to one skin.

**Unbound meshes vanish from the import.** When the GLB has a skin, Interchange's default pipeline drops every mesh node that has no `skin`, no morph targets and no skin joint above it, with no warning and no StaticMesh (`bAutoDetectMeshType` sets `bIgnoreStaticMeshes` in `InterchangeGenericMeshPipeline.cpp`, UE 5.7). The usual way to get there is building eyeballs through `agent-meshes build` with a `blender_lib` source and not passing them to `bind_skin`: the head imports with no eyes, and the only trace is an `Eye` material that nothing uses. So `verify-unreal` accounts for geometry on every run. The pre-flight counts each mesh node's vertices the way Unreal's mesh build keeps them: only vertices a triangle uses, welded when position (to 0.1 mm), normal (to 0.01) and first UV (to 1/1024) agree, with points and lines left out (`geometry.glbVertices`). It compares that with LOD 0 of every SkeletalMesh and StaticMesh Unreal made (`geometry.unrealVertices`). On UE 5.7.3 these matched exactly for SkeletalMeshes, including a sphere whose repeated pole and seam vertices Unreal welds (185 used, 146 imported). A StaticMesh build welded 4 more of an eyeball's 1104 vertices, so up to 1% may go missing (`geometry.tolerance`). A larger loss fails. When the pre-flight finds mesh nodes Interchange is known to drop (`geometry.droppedByInterchange`), the failure names them with their vertex counts and says to bind them to the rig's skin. That also applies when they are small enough to hide inside the tolerance: the run fails once at least half of their vertices are missing. A mesh parented under a joint (an eyeball node that is a child of `eye_L`) is merged into the SkeletalMesh and passes. When the vertex data cannot be read (a `.gltf` with external buffers, or sparse or Draco-compressed accessors), the check is skipped with a warning.

**Bone hierarchy.** With `--contract arkit-face/1`, `head` must be the root of the SkeletalMesh's reference skeleton and `eye_L`/`eye_R` must be its children. Names alone are not enough, so a flat Blender armature, or eye bones parented to each other, fails. The failure quotes Unreal's parents and the GLB skin's joint parents. When a skin has several root joints, Interchange adds a root bone named `<armature>_ProxyTrueRootJoint` above them. A proxy root directly above `head` is allowed, because it changes nothing for the rig. Eye bones under the proxy fail. When Unreal's parents are unavailable, the GLB's joint parents are checked instead.

**Several skins.** The contract asks for a single skin. When a GLB binds its meshes to more than one skin but Unreal still produced one correct SkeletalMesh (the skins shared joint nodes), the run passes with a `warnings` entry naming the skins, since other importers may not merge them. When Unreal splits them into several SkeletalMeshes, the run fails as described above.
...
```python
from agent_meshes_author import (face_skeleton, build_eye, eye_holes, eye_hole_mask, JawHinge, add_jaw_open, slit_mouth,
    front_surface, teeth_row_geometry, mouth_cavity_geometry, mesh_from_geometry, join_face_parts, face_contract)

# A sketch: tests/fixtures/face-rig/test_head.py is the complete, runnable source.
def build():
    eyes_at = {'L': (.033, -.067, .145), 'R': (-.033, -.067, .145)}
    rig = face_skeleton(head=(0, 0, .09), eye_left=eyes_at['L'], eye_right=eyes_at['R'])
    cut = eye_holes(vertices, faces, eyes_at['L'], .014, opening=(45, 38, 30))   # the skin itself becomes the lids
    vertices, faces, holes = cut['vertices'], cut['faces'], {'L': cut['L'], 'R': cut['R']}
    skin = ...                                   # a mesh of vertices/faces; slit_mouth(skin, .075, .022); brow and cheek
                                                 # offsets with mask=eye_hole_mask(*holes.values())
    jaw = JawHinge.ear(vertices, .075, .022)     # hinged at the back of the head, level with the mouth
    add_jaw_open(skin, jaw, min_chin_drop=.1)    # the chin drops >= 10% of the head, not only the lips
    ...
    eyes = [build_eye(rig, side, eyes_at[side], .014, hole=holes[side], skin=skin, lash=True) for side in 'LR']   # lid shape keys on the skin
    face = join_face_parts([skin, lower, *(e['lids'] for e in eyes)], 'face', rig=rig)
    face_contract(rig, [face], yaw_max=25, pitch_max=18, exposed_teeth=[])
    return [rig, face, *(e['eyeball'] for e in eyes)]
```

- **Eye holes.** For lid eyes, `eye_hole(vertices, faces, center, radius, **lid_options)` (or `eye_holes` for both eyes of a symmetric head, as exact mirror images) rebuilds the skin round the eye as one surface that is the lids: the skin flows over the eyeball as the upper and lower lids, each margin turns inward round a half-round as thick as the lid onto the lid's inner surface, a fornix and a lining that wraps the eyeball, the two margins meet in one point at each corner, and one soft crease lies above the upper lid. `build_eye(..., hole=..., skin=head)` adds `eyeBlink`, `eyeSquint` and `eyeWide` to the head as shape keys of those lid rows and paints the lash line and the lining on it. The lid radius is solved so no lid vertex enters the eyeball at any blink, squint or wide mix, the closed upper lid passes in front of the lower so blink 1 + wide 1 stays shut from below too, and every ray through the opening meets the eyeball, a lid or the lining. This replaced separate lid shells stitched into a hole in the skin (still available as `style='shells'`): their rim, bevel, mound ring, lid shells and lash band read as 3-4 stacked bands round every eye (the P1a round 6-7 critics' terraced sockets), which the verifier's `eye-crease` check now fails. Skin shapes near the eyes (brows, cheeks) take `mask=eye_hole_mask(*holes)` so they leave the moving lid rows still. Shutter eyes in a flat face plate take `shutter_hole`, which clips the plate round the gaze axis, runs a tube straight back that the blades slide through and caps it behind the eye (a plain `cut_hole` in a single-layer tin skin let the round-3 Bolt's view from below look into the head); `build_eye(..., style='shutter', hole=...)` adds a housing in the blades' material.
- **Eyes.** `build_eye` makes an eyeball with `eye_white`/`eye_iris`/`eye_pupil` slots bound to its eye bone (overrides must keep those names), lids (`style='lid'`) or robot shutters (`style='shutter'`) carrying the blink, squint and wide morphs, and a housing (shutters) or, for a lid eye without `hole`, a dark socket cup. Morphs are linear, so a lid swept across a round eye cuts a chord through the eyeball mid-blink. The lid helper solves the shell radius so that every blink, squint and wide weight combination keeps every lid vertex at least 0.5 mm outside the eyeball (the lower lid rises to meet a limited upper arc, and the upper lid passes in front); shutters translate in planes in front of the lens. Morphs also add: blink 1 + squint 1 pushes each lid past the meet line by the squint travel, and blink 1 + wide 1 pulls the closed lids apart by the wide travel (surprised plus an idle blink). So `shutter_geometry` sizes the blades and their overlap for every blink, squint and wide weight in [0, 1]^3 and rejects an explicit `blade_height` or `overlap` that cannot cover the eyeball, and `lid_geometry` raises the closed lower lid behind the upper one by the least amount that keeps blink + wide closed without folding. Both run `eye_coverage_problems`: front rays across the eyeball disk must all hit a lid at every full blink (alone, with squint 1, with wide 1) and may show only what neutral shows at blink .25/.5/.75 and squint. Tall blades stick out past the eye, so pass the skin's `surface=front_surface(...)` and `shutter_geometry` rejects blades that would slide out through the face: flat shutters belong behind a flat face with the eyes recessed (a tin can, `ellipsoid_geometry(..., exponent=6)`). The helpers also reject squints outside 25-60% of the opening and lid settings that fold faces.
- **Jaw.** One `JawHinge` drives `jawOpen` on everything the jaw carries, as a puppet jaw: the chin and lower face outline drop with the teeth, not only the lips. **Put the pivot at the back of the head, level with the mouth line (by the ears).** `JawHinge.ear(head_vertices, mouth_z, half_width)` places it there and sizes the motion (an 8 degree turn plus a downward `drop` of 9% of the head height), which drops the chin by 12-14% of the head on the fixtures. A pivot in the middle of the head swings the chin back and up and only opens a hole at the lips. Below the mouth line the whole lower face moves. On the lower lip the weight follows a rounded mouth shape (full at the middle, none at the corners), and beyond the corners and behind the slit it fades in over `band`, so the cheeks stretch without folding. `add_jaw_open(skin, jaw, min_chin_drop=.1)` rejects a jaw whose lowest point drops by less than 10% of the part's height, and every call rejects a jaw that folds faces. `rigid=True` moves a whole part as one body (lower teeth, tongue, a robot chin plate).
- **Mouth.** `slit_mouth` cuts a closed lip seam and tags its vertices (the `jaw_seam` point attribute: 1 lower lip, 2 upper lip), so the lower lip opens and the upper lip stays whatever float32 does to the mouth line (.088 rounds down, .086 up; both work). It first slides any vertex a hair off the line onto it along its crossing edge, so the cut leaves no sliver row to shade as a seam across the face or fold at the corners. `sculpt_lips(vertices, faces, mouth_z, half_width)` gives a skin face soft lips and a lip line at rest before the slit is cut. `mouth_cavity_geometry` makes a dark bag so an open mouth never sees through the head. Pass `surface=front_surface(skin_vertices, skin_faces)` and its rim follows a curved face 4 mm behind the skin instead of poking out at the cheeks. `tongue_geometry` makes a tongue, `teeth_row_geometry` rows of rounded, saw (pointed) or grille (rectangular) teeth, and `exposed_teeth_geometry(surface, xs, mouth_z, length, width)` fangs or buck teeth that hang in front of the closed lower lip. Give those their own material, `teeth_exposed`, and declare it: `exposed_teeth=['teeth_exposed']`. The verifier then tells them from a row poking through the lips.
- **Skin holes and brows.** `cut_hole(vertices, faces, center, radius)` opens a round or elliptical hole (a mouth or other opening; eyes take `eye_hole` or `shutter_hole`) whose rim lies exactly on the circle and returns a dict (`vertices`, `faces`, `mapping`, `source`, `origin`, `boundary`); `cut_faces` returns a 3-tuple `(vertices, faces, mapping)` and leaves stair steps. Brows: on a skin-faced head (a kid), `skin_brow_geometry(head, side, inner, outer, hole=...)` lays a hair-coloured brow on the skin itself (each cross-section settled along the skin's normal, its edges sunk in) whose `browDown` (which also knits the inner end), `browInnerUp` and `browOuterUp` morphs slide it over the skin, laid on the skin in each of the head's own shape keys; on eyes in domes (a frog), `brow_ridge_geometry(eye_center, hole['mound'], side, skin=..., hole=...)` lays a heavy ridge on the skin itself, sunk into it like a fold, and its morphs slide it over the skin; a robot takes `brow_plate_geometry`.
- **Skin paint.** `skin_tints(vertices, base, patches, mottle)` computes blush, lips, lash lines, socket shading and mottling as per-vertex tints; `paint_vertices(obj, tints)` stores them and `use_vertex_colors(mat)` multiplies the material's color by them. `join_face_parts` carries color attributes through the join. They export as glTF `COLOR_0`; three.js and Unreal's imported glTF material (`M_Default`, which reads vertex color in `MF_BaseColor`) both apply them, and `verify-unreal` reports `hasVertexColors`.
- **Noses and soft forms.** `nose_geometry(vertices, faces, tip, size)` sculpts a round button nose with nostril dimples (material index 1) into the skin as a smooth union with a filleted base, refined so it stays smooth, and returns the wing lift for `noseSneer` (`symmetric_offsets(rest, *nose['sneer'])`); `sculpt_skin(vertices, faces, shapes)` grows any ellipsoids (cheeks, chin, snout) out of a blank the same way. `sdf_blank(sdf, center)` samples a whole head built as a signed-distance field (`ellipsoid_sdf`, `smooth_min`, `smooth_max`) into a closed, symmetric blank, densest over the face. A `soft_offset` sculpt on a blank's 6-8 mm faces makes a spike.
- **Small parts on the skin.** `attach_to_skin(part, head, depth=...)` seats a nostril, freckle, wart or horn on the skin and gives it the skin's own morph deltas at its footprint, so `noseSneer` lifts the nostrils with the snout and `jawOpen` carries a chin wart; `skin_contact(part, skin)` measures a part as the `attached-parts` check does. A part can sit on another attached part (nostrils attached to a nose ball with `attach_to_skin(bead, ball)`). See `scripts/blender_lib/README.md`.
- **Robot plates.** For a head like Bolt's: `split_plates(vertices, faces, split_z, thickness, gum_z=...)` cuts a closed head at a plane into a skull and a chin plate, each a closed shell with an inner wall and a flat rim band `thickness` wide, so an open jaw shows solid edges, not a paper-thin open shell; it rejects a split that would put the chin plate's rim above the upper teeth's gum line (the `upper-lip` check). `brow_plate_geometry(center, (width, depth, height), side, surface=front)` is a rigid brow bar whose `browDown`, `browInnerUp` and `browOuterUp` morphs turn it about its ends, standing off the face at every weight. `rubber_mouth_geometry(front, mouth_z, half_width, jaw=jaw)` is the rubber mouth edge between the plates: two tubes that carry `mouthSmile`, `mouthFrown`, `mouthStretch` (each side) and `mouthFunnel` while the plates stay rigid, with the lower tube riding `jawOpen`. `tests/fixtures/face-rig/test_robot.py` builds a whole tin-can robot from these.
...
| `eyes` | no mesh is bound 100% (>= 0.999) to an eye bone, the eye bone sits more than 1 mm from that eyeball's center, or a part too large to be an eyeball (reaching past half the distance between the eyes, such as a skull) is bound to an eye bone |
...
| `inversion` | any triangle flips or collapses to under 0.1% of its area at each morph = 0.5 and 1, or at each emotion preset (with lid follow) alone and with `jawOpen` = 1. The message says where: the rest-pose centers (mm, glTF coordinates) of up to four spots the bad triangles gather in (within 5 mm), each with its face region (brow, eye, cheek, nose, side of the mouth, chin; the character's left or right), for example `happy preset + jawOpen=1 flips 2 triangle(s) on face[skin] at (19.4, 87.8, 79.3) mm (left side of the mouth), ...`; `measurements.inversions` lists the combination, part, count and spots (m) for up to 20 of them |
...
| `eye-coverage` | front rays cast across each eyeball (a 48-ray-wide grid, through lids, skin and everything else) reach the eyeball at `eyeBlinkX` = 1 alone, with `eyeSquintX` = 1, with `eyeWideX` = 1 or with both (a full blink must hide the whole eyeball), or reach it at blink .25/.5/.75 (alone and with squint 1) or squint 1 where the neutral opening hides it (eyeball over a lid). Every closed state is also cast from 25 and 15 degrees below and 15 above (`COVERAGE_PITCHES`: the eyeball and every opaque occluder, lash bands included, turned about the eye's X axis): closed lids that overlap seen from the front can still part by a strip seen from below when one stands in front of the other (round 6's kid showed its iris at blink 1 + wide 1 from 25 degrees below, 0.28-0.32 eyeball radii above its center), and that fails when the front view sees the state closed. Only rays within 0.6 eyeball radii of the eye's center height in that view count (`COVERAGE_PITCH_BAND`, where closed lids meet): the eyeball's rim showing at a shutter housing from an angle (0.7-0.99) is left to `eye-oblique`. Clearance alone misses this: a lid far from the eye can still leave it bare. `measurements.eyes.<side>.coverage` lists the rays that reach the eyeball in every state, and `coverage.pitched[pitch][state]` those that reach it in each closed state from each pitch |
| `eye-oblique` | a ray around an eye reaches the socket cup or the inside of the head. Rays are cast in parallel from 15 directions (the front, 3/4 at 35 and 45 degrees of yaw each way, and each of those 20 degrees above and below) at a 64-ray-wide disk 1.6-2 eyeball radii around each eye, in every coverage state, lid follow at full gaze and every emotion preset (brows and cheeks move the rim). A leak is a first hit on a part named `socket`, or on a back face behind the eye's center that lies within 1.8 eyeball radii of it or was reached through the eye hole (a ray passing within 1.2 radii). This catches what a front view cannot: round 3's 3/4 black hole beside the lids. The message tells a gap at the lids from a hole elsewhere in the skin: leaks whose rays pass no lid (checked 3 rays each way in that view) are reported as a hole in the skin away from the lids, which is where the fix belongs. `measurements.eyes.<side>.oblique` reports the rays, leaks and worst state and view |
| `eye-crease` | a lid eye whose socket is terraced: along 17 radial lines above and 17 below each eye in a front view, from the lid margin out to 1.3 eyeball radii (lines stop behind the eye's center, where a dome turns over it), the surface's slope is traced and every place it turns back toward the viewer by 20 degrees within 0.06 eyeball radii of surface is a fold (a crease, or a step onto another surface). The median line may fold once above (the lid crease) and once below (where the lower lid meets a full cheek; on a plain face it does not); the round-7 shell-lid eyes fold two to three times above. Attached parts (brows, ridges) are left out, so the lines read the skin under them; shutter eyes (translating blades) are not judged. `measurements.eyes.<side>.crease` lists each line's folds and where they lie, in eyeball radii from the eye center |
| `lid-follow` | a thin column of front rays down the middle of each eye finds the upper lid's edge moving less than 1.5 mm (about 2 px at 512 px) up at `eyeWideX` = `lidFollow.up` (eyeLookUp = 1) or down at `eyeBlinkX` = `lidFollow.down` (eyeLookDown = 1), or no upper lid showing at rest. When the eyeball just above the raised edge is hidden by something other than the lid (a hair fringe, a brow, a skin rim), the message names that part: more `lidFollow.up` or wide travel cannot help, moving the part can. `measurements.eyes.<side>.lidFollow` reports both in meters |
| `materials` | a face part (a morph-bearing primitive, or one named skin, lid, teeth, face, skull, socket, cavity or tongue) uses an alpha-blended or alpha-masked material or `KHR_materials_transmission`: a see-through lid shows the eye open while its geometry says closed. Every coverage and tooth ray check also treats such materials as hiding nothing. Only eyeballs may be transparent |
| `attached-parts` | a small part joined to the face floats off the skin or sinks into it, at rest or at any morph at weight 1 that moves it or the skin near it. Parts are found by geometry: the opaque surfaces are welded by position into pieces; teeth, tongue and cavity, eyeballs, lids (anything an eye morph moves), sockets and pieces wholly inside an eye's lid reach (a shutter housing) are left out, pieces at least a third the size of the largest one are skin (head, skull, chin plate, hair cap), and every other piece is an attached part, judged against the skin and the lids; a part that fails there but touches a larger attached part that passes (a nostril on a nose ball) is judged against that host too, and `measurements.attached` names it as `host`. A part on the face (in front of the eyes' depth less an eyeball radius: brows, ridges, nostrils, freckles) is cut into slices along its length by footprint (each sample placed by its nearest skin point, so a thick ridge curved round a dome keeps its top in the same slice as its base; slicing the samples themselves along the chord left the end slices holding only the ridge's top), one per 1.5 mm, at most 32, and every slice must come within 0.5 mm of the skin or dip into it: round 4's brow ridge hung 3.1 mm over the dome along all of its 18 slices. Samples farther than 12 mm from the skin (the top of a very heavy ridge) join the nearest end slice. Parts are classified by placement and shape, not height: an appendage sits beside or behind the eyes (an ear, a fin), or rises out of the skin more than 3 mm and further than its base spreads along the skin (a horn at a brow corner; the rise is measured from the skin, or from the part's own underside when it floats, and the base is the footprint of its lowest third), and it may stand out but must touch the head somewhere; everything else lies on the face, a ridge however heavy (round 6's 12 mm frog ridges on a 25 mm base). A part seated in the skin must also be sealed across its width at rest: in every slice that dips more than 0.5 mm into the skin it sits in (not a lid, not another piece it merely hangs over, such as the face under a nostril on a nose ball), its surface that faces that skin may stand out of it by at most 0.5 mm, or light shows under its edge (round 6's frog ridges pushed 2 mm off their domes still dipped into them in every slice, but their undersides hung 0.70 mm off where they were seated). A part resting on the skin without dipping in (a robot's round rubber mouth tube laid 0.3 mm in front of the plates, a bead sitting on it) is judged by its gap alone: its underside curves away by its own shape. A morph that leaves less than half as much of a part above the skin as at rest buries it (a nostril that does not carry the skin's `noseSneer`). A morph must not slide the skin away under a part that stays: where the part touches the skin at rest (vertices within 0.5 mm, lids left out), the skin's own delta at that footprint is compared with the part's, and more than 0.5 mm of difference where the part moves less than half as far as the skin fails (round 6's static nostril, 1.49 mm on `noseSneerLeft`); attach_to_skin gives each part vertex exactly that delta, and brows that slide over the skin by their own morphs move too far to count. The message names the piece by its primitive, position and morphs, and the fix (`brow_ridge_geometry(..., skin=...)`, `attach_to_skin`). `measurements.attached` lists every part with its rest and worst gap, its `contact` (`lies` or `root`), `rise`, `base`, `overhang` and worst `slide` |
...
| `exposed-teeth` | `extras.arkitFace.exposedTeeth` is not a list of names (`[]` when no teeth show at rest); a listed name is not an exposed-teeth material or mesh (its name must contain `exposed`, as in `teeth_exposed`: a whole row such as `teeth_upper` is rejected); or front rays over the teeth at rest land on a tooth part that is not a declared exposed-teeth part before the lips or jaw plate (a row pushed through a closed face). `measurements.restTeeth` counts the rays that land on teeth |
| `head-binding` | a morph-bearing part, or a part named skull, gum, teeth, head, skin, face or lid, is not bound 100% to `head` (the message names the bone it is bound to) |
| `teeth` | no upper or lower teeth are found; `jawOpen` moves the upper teeth by 0.1 mm or more; or `jawOpen` lowers the lower teeth by less than 1 mm |
...
- The **upper teeth** and **lower teeth** are primitives whose material (or mesh or node) name contains `teeth` or `tooth` and `upper` or `lower`: name the materials `teeth_upper` and `teeth_lower`. **Exposed teeth** (shown at rest) have `exposed` in the name too: `teeth_exposed` counts with the upper row (it must stay on the skull), `teeth_exposed_lower` with the lower.
- The **socket** is a primitive named with `socket`: no ray may see it. An eye housing (`eye_housing_<side>`) is a visible part.
- A primitive whose material is alpha-blended, alpha-masked or transmissive hides nothing in any ray check.
- The **mouth parts** are primitives named with `tongue`, or `cavity`, `throat` or `mouth_interior`.
- The **face** for the chin measurement is every other primitive except eye sockets and parts named `neck`, `collar`, `body`, `torso` or `shoulder`, which do not open with the jaw. Its lowest point is the chin. `measurements` reports `chinDrop`, `faceHeight`, `chinDropRatio`, `upperLipMove` and what the `mouthOpen` rays hit.

The eye-coverage and exposed-teeth rays approximate a front render geometrically; the clauses that need a real render (a dark open mouth, gaze coverage, shading) are not checked here. The required names come from `src/arkit-face.ts`, the one copy every verifier imports, and the `skeleton`, `skinning` and `morph-names` checks reuse the same skin and morph-name audits as `verify-unreal` (`src/gltf-skins.ts` `auditSkins`, `src/gltf-morphs.ts` `morphNameAudit`), so both verifiers judge a layout the same way. `src/gltf-read.ts` is the small GLB reader the verifier uses (accessors, including sparse ones, skins and node world matrices).

**Fixtures and proofs.** `tests/fixtures/face-rig/test_head.py` (round eyes whose lids are the skin, from `eye_holes`, an ear-hinged puppet jaw, rounded teeth and a tongue), `test_robot.py` (Bolt's tin can: recessed shutter eyes in `shutter_hole` tubes, brow plates, a skull and chin plate with thick edges from `split_plates`, grille teeth and a rubber mouth edge), `test_frog.py` (a wide, flat frog with eyes in mounded `eye_hole` domes, brow ridges lying on the skin, nostrils that ride `noseSneer` through `attach_to_skin`, saw teeth, two `teeth_exposed` fangs and a mouth line at .088) and `test_kid.py` (a round-faced kid with `skin_brow_geometry` brows lying on the skin, a fused `nose_geometry` nose, freckles and buck teeth) are built only with the helpers, and all four pass every check with no oblique leak. `tests/face-rig-blender.test.ts` builds all four through `agent-meshes build` and verifies them when Blender is installed; CI skips it. `tests/face-contract.test.ts` builds small GLBs in Node and proves each check passes on a good head and fails on a broken one. `node scripts/check-face-rig-browser.mjs [test_head|test_robot|test_frog|test_kid]` builds a fixture, verifies it, and renders blink, squint, wide, jaw and emotion states through the offline viewer into `.agent-meshes/face-rig-proof/<fixture>/contact-sheet.png`, plus `jaw-sheet.png` (jawOpen 0, .5 and 1 from the front, three-quarter and close up), captioned with the verifier's chin drop and E3 measured in pixels on an exact ID render (`viewer.idRender`): the silhouette's lowest point, and the lower- and upper-teeth centroids drawn unoccluded. A multi-material face loads in MeshViewer as one three.js mesh per primitive; its node name is listed in `morphGroups`, and `setMorph('face', target, weight)` drives that target on every primitive at once (the proof script does this).

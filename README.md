...
```python
from agent_meshes_author import (face_skeleton, build_eye, eye_hole, eye_hole_mask, JawHinge, add_jaw_open, slit_mouth,
...
def build():
    ...
    holes = {}
    for side, c in eyes_at.items():               # eye holes shaped from the lids: the skin meets them all the way round
        holes[side] = eye_hole(vertices, faces, c, .014, opening=(45, 38, 30))
    ...
    eyes = [build_eye(rig, side, eyes_at[side], .014, hole=holes[side]) for side in 'LR']
...
- **Eye holes.** For lid eyes, open the skin with `eye_hole(vertices, faces, center, radius, **lid_options)` and pass the result to `build_eye(..., hole=...)`. It shapes the hole from the lids instead of cutting a sphere: it mounds the skin over them (they never poke out), draws far skin in to a socket around them, clips the skin along the lids' window (their opening plus every edge's travel, a few degrees wider, with round ends), and walls the rim down under the lids into a lining that wraps the eyeball. The skin stays one closed surface, so no view from the front, 3/4 or above and below looks past the lids into the head, in any blink, squint, wide or emotion state. Round 3 showed why: a `cut_hole` 1.32 eyeball radii wide left a black hole beside each eye in 3/4 view, between the lids' cut-off corners and the rim, and no radius fixes it. Skin shapes near the eyes (brows, cheeks) take `mask=eye_hole_mask(*holes)` so they leave the rim and wall still. `eye_holes(vertices, faces, eye_left, radius, **lid_options)` cuts both holes of a symmetric head as exact mirror images (two `eye_hole` calls in a row are not). The mound blends into the face over half an eyeball radius and the rim's bevel turns evenly, so the eye reads as one soft crease; `build_eye(..., lash=True)` adds a crisp lash line along the upper lid's edge. Shutter eyes in a flat face plate take `shutter_hole`, which clips the plate round the gaze axis, runs a tube straight back that the blades slide through and caps it behind the eye (a plain `cut_hole` in a single-layer tin skin let the round-3 Bolt's view from below look into the head); `build_eye(..., style='shutter', hole=...)` adds a housing in the blades' material.

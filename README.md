...
```python
from agent_meshes_author import (face_skeleton, build_eye, JawHinge, add_jaw_open, slit_mouth, cut_hole,
...
def build():
    ...
    skin = ...                                   # your head mesh, eye holes cut with cut_hole; slit_mouth(skin, .075, .022)
    jaw = JawHinge.ear(head_vertices, .075, .022) # hinged at the back of the head, level with the mouth
...
| `exposed-teeth` | `extras.arkitFace.exposedTeeth` is not a list of names (`[]` when no teeth show at rest), or it is empty and front rays over the teeth rows at rest land on teeth before the lips or jaw plate (teeth pushed through a closed face). `measurements.restTeeth` counts those rays |

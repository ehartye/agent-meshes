...
for (const kind of creatureKinds) {
  ...
  console.log(`${creatureInfo[kind].name}: ${built.files.length} files → ${built.output}`);
}
...
const cards = creatureKinds.map(kind => {
  ...
  return `<article style="--creature:${info.color}">
    ...
    <div class="caption"><div><p class="body-plan">${info.label} <span>· ${info.legs} legs</span></p><h2><a href="${kind}/preview.html">${info.name}</a></h2><p>${info.description}</p></div><span class="gait">${info.gait}</span></div>
    <div class="links" aria-label="${info.name} files"><a href="${kind}/front.png">Front</a><a href="${kind}/side.png">Side</a><a href="${kind}/${info.gait}-contact.png">Motion sheet</a><a href="${kind}/model.glb" download>Download model</a><a href="${kind}/project.mesh.json" download>Editable project</a></div>
  </article>`;
}).join('\n');
...
</style></head><body><main><div class="brand"><i aria-hidden="true"></i> Agent Meshes <span aria-hidden="true">/</span> Creature studies</div><header><h1>Four ways<br>to <em>move.</em></h1><p>Meet four articulated creatures. Watch their gaits, orbit each model, or take one into your next scene.</p></header><section class="collection" aria-label="Animated creatures">${cards}</section><footer><p>Two, four, six, eight legs. A different rhythm for each.</p><p>Previews work offline. Drag to orbit; scroll to zoom.</p></footer></main></body></html>`);
